...
class MDSRPlusModel(tf.keras.Model):
...
def make_mdsr_plus_model(num_filters=64, num_of_residual_blocks=32):
    ...
    outputs = []
    for scale in [2, 3, 4]:
        upsampled = Upsampling(x, factor=scale, num_filters=num_filters)
        upsampled = layers.Conv2D(3, 3, padding="same")(upsampled)
        outputs.append(layers.Rescaling(scale=255)(upsampled))

    return MDSRPlusModel(input_layer, outputs)

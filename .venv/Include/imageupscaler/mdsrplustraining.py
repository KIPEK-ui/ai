...
from mdsrplus import make_mdsr_plus_model
...
def train_model(scale, epochs=100, batch_size=16):
    ...
    model = make_mdsr_plus_model(num_filters=64, num_of_residual_blocks=32)
    optim_mdsr_plus = tf.keras.optimizers.Adam(
        learning_rate=tf.keras.optimizers.schedules.PiecewiseConstantDecay(
            boundaries=[5000], values=[1e-4, 5e-5]
        )
    )
    model.compile(optimizer=optim_mdsr_plus, loss="mae", metrics=[PSNR])

    model.fit(train_ds, epochs=100, steps_per_epoch=200, validation_data=val_ds)
    
    # Save the model as a .h5 file
    #model.save('mdsrplus_model.h5')
    # Save the model in the .keras format
    model.save('mdsrplus_model.keras', save_format='keras')
